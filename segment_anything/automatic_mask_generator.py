...
class SamAutomaticMaskGenerator:
    ...
    def _generate_similarity(self, image, ref_prompt):
        ...
        for i, ref_mask in enumerate(masks_low_res.cpu()):
            ...
            ref_mask_all = ref_mask if i == 0 else ref_mask_all + ref_mask
            ...
            if target_feat.shape[0]>0:
                ...
                fsim_ = target_feat @ test_feat
                fsim_mask = fsim_.reshape(h, w) if i == 0 else fsim_mask + fsim_.reshape(h, w)
                fsim_ = fsim_.reshape(1, 1, h, w)
                fsim_ = F.interpolate(fsim_, scale_factor=4, mode="bilinear")
                fsim_ = self.predictor.model.postprocess_masks(
                                fsim_,
                                input_size=self.predictor.input_size,
                                original_size=self.predictor.original_size).squeeze()#"""
                fsim_ = fsim_.cpu().numpy()
                fsim.append(fsim_)


        # """ background context
        fsim_mask = fsim_mask.cpu().numpy()
        T = np.max(fsim_mask)/1.3 # threshold T1
        fsim_mask[fsim_mask<T]=0
        fsim_mask[fsim_mask>=T]=1
        ref_mask_all = ref_mask_all + fsim_mask
        
        background_feat = ref_feat[ref_mask_all <= 0] 
        background_embedding_ = background_feat.mean(0).unsqueeze(0) 
        background_feat = background_embedding_ / background_embedding_.norm(dim=-1, keepdim=True) 
        background_embedding_ = background_embedding_.unsqueeze(0) 
        
        bsim = []
        bsim_ = background_feat @ test_feat 
        bsim_ = bsim_.reshape(1, 1, h, w) 
        bsim_ = F.interpolate(bsim_, scale_factor=4, mode="bilinear") 
        bsim_ = self.predictor.model.postprocess_masks(
                        bsim_,
                        input_size=self.predictor.input_size, 
                        original_size=self.predictor.original_size).squeeze()
        bsim_ = bsim_.cpu().numpy()
        bsim.append(bsim_)
        # """
        
        # weighted fusion
        fsim = np.array(fsim).mean(0) if self.fusion_type == 'mean' else np.array(fsim).max(0)
        bsim = np.array(bsim).mean(0)
        foreground_ratio = torch.sum(ref_mask_all > 0).float() / ref_mask_all.numel() 
        sim = fsim if foreground_ratio > 0.5 else fsim - self.fusion_ratio * bsim
        target_embedding = torch.mean(torch.cat(target_embedding, dim=0), dim=0, keepdim=True)
    ...
    def _generate_masks(self, image: np.ndarray,ref_prompt) -> MaskData:
        ...
        #"""
        T = np.max(sim_map)/1.3 # threshold T2
        sim_map[sim_map<T]=0
        sim_map[sim_map>=T]=1 #"""
        
        data = MaskData() 

        #""" matrix point prompts
        ...
        matrix_point_prompts = self.point_grids * points_scale
        point_mask = np.zeros(orig_size, dtype=bool) # generated masks
        ## Generate masks for this crop in batches
        for (points,) in batch_iterator(self.points_per_batch, matrix_point_prompts):
            points_new = []

            points = points.astype(int)
            valid_points_mask = np.array(point_mask[points[:, 1], points[:, 0]] == 0)
            points_new = points[valid_points_mask]

            if len(points_new)>0:
                batch_data, point_mask = self._process_batch(True
                                                            , np.array(points_new)
                                                            , None
                                                            , orig_size
                                                            , sim_map
        ...
        # """ residual point prompts
        residual_point_prompts = []
        height_ratio = point_mask.shape[0] / sim_map.shape[0]
        width_ratio = point_mask.shape[1] / sim_map.shape[1]

        local_maxi_coords = np.argwhere(sim_map)
        scaled_coords = local_maxi_coords * [height_ratio, width_ratio]
        scaled_coords = scaled_coords.astype(int) 
        valid_points_mask = np.array(point_mask[scaled_coords[:, 0], scaled_coords[:, 1]] == 0)
        residual_point_prompts = local_maxi_coords[valid_points_mask]

        ## Generate masks for this crop in batches
        for (points,) in batch_iterator(self.points_per_batch, residual_point_prompts):
            scaled_coords = points * [height_ratio, width_ratio]
            scaled_coords = scaled_coords.astype(int) 
            valid_points_mask = np.array(point_mask[scaled_coords[:, 0], scaled_coords[:, 1]] == 0)
            points_new = points[valid_points_mask]
            labels_new = np.ones(points_new.shape[0], dtype=int)

            if len(points_new)>0:
                
                batch_data, point_mask = self._process_batch(False
                                                            , points_new[:, ::-1].copy()
                                                            , labels_new
                                                            , orig_size
                                                            , sim_map
    ...
    def _process_batch(
        ...
        if matrix_point_prompts:
            ...
            in_points = torch.as_tensor(points, dtype=torch.int, device=self.predictor.device)
            in_labels = torch.as_tensor(label, dtype=torch.int, device=in_points.device)
